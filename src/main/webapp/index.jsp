...
<body>
	...
	<script type="text/javascript">
		var totalRecord, crrenPage;//全局总记录数
		$(function () {
			to_page(1);
		});
		...
		$(document).on("click",".delete-btn",function(){
			...
			if (confirm('您确认要删除该信息吗？')) {
				$.ajax({
					url : '${APP_PATH}/deleteStudent/'+stuId,
					type : 'DELETE',
					success : function (result) {
						to_page(crrenPage);
					}
		...
		$('#check_all').click(function(){
			//arrt只能获取自定义的属性的值，原生属性值只能用prop来获取
		...
		$('#stu_del_all_btn').click(function (){
			var checkedItem = $('.check_item:checked');
			var del_idstr = '';
			...
			if (confirm('您确认要删除以下数据信息吗？')) {
				$.ajax({
					url : '${APP_PATH}/deleteStudent/'+del_idstr,
					type : 'DELETE',
					success : function (result) {
						if (result.code == 100) {
							alert('删除成功！');
